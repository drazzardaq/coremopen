...
watch(
  ...
  ([search, sorting, includeArchive]) => {
    flowbiteInit();
    router.get(route("dashboard"), { search: search, sortBy: sorting.field, direction: sorting.direction, includeArchive: includeArchive }, { preserveState: true, replace: true });
  },
  { deep: true }
);
          ...
          <tbody>
            <template v-for="item in $props.items.data" :key="item.id">
              <tr>
                ...
                <td>
                  <FontAwesomeIcon icon="eye" class="text-primary/30 text-sm md:text-base lg:text-lg mr-2 focus:ring-0 focus:text-accent hover:text-accent transition cursor-pointer" :data-tooltip-target="'tooltip-details' + item.id" @click="actions(item.id, 'read')" />
                  ...
                  <FontAwesomeIcon icon="edit" class="text-primary/30 text-sm md:text-base lg:text-lg mr-2 focus:ring-0 focus:text-accent hover:text-accent transition cursor-pointer" :data-tooltip-target="'tooltip-update' + item.id" @click="actions(item.id, 'update')" />
                  ...
                  <FontAwesomeIcon :icon="item.archived ? 'folder-minus' : 'folder-plus'" class="text-primary/30 text-sm md:text-base lg:text-lg mr-2 focus:ring-0 focus:text-accent hover:text-accent transition cursor-pointer" :data-tooltip-target="'tooltip-archive' + item.id" @click="actions(item.id, 'archive')" />
                  ...
                  <FontAwesomeIcon icon="trash" class="text-primary/30 text-sm md:text-base lg:text-lg focus:ring-0 focus:text-accent hover:text-accent transition cursor-pointer" :data-tooltip-target="'tooltip-trash' + item.id" @click="actions(item.id, 'delete')" />
    ...
    <DialogModal :show="modal.active" @close="closeModal">
      <template #title>
        ...
        <!-- <div class="flex w-full items-center">
					<span v-if="modal.button === ('Read' || 'Update')">{{ edit ? 'View Item' : 'Edit Item' }}</span>
					<span v-else>{{ modal.title }}</span>
					<FontAwesomeIcon v-if="modal.button === ('Read' || 'Update')" :icon="edit ? 'edit' : 'eye'" class="ml-auto text-primary/60 text-sm md:text-base lg:text-lg focus:ring-0 focus:text-accent hover:text-accent transition cursor-pointer" @click="toggleEdit" />
				</div> -->
      </template>
        ...
        <div class="my-4 text-sm text-gray-600 px-6 py-4">
          ...
          <template v-if="modal.button != 'Delete'">
            <div class="grid grid-cols-2 gap-6 w-full">
              ...
              <div class="flex flex-col">
                ...
                <select id="category_id" v-model="activeRow.category_id" :disabled="modal.button === 'Read'" class="border-gray-300 focus:border-blue-500 focus:ring-0 rounded-lg shadow focus:shadow-sm duration-500">
                  ...
                  <option :value="3" disabled>Projects</option>
                  <option :value="4">Tasks</option>
                  <option :value="5" disabled>Finances</option>
                  <option :value="6" disabled>Identity</option>
                </select>
              </div>
            </div>
            ...
            <div class="mt-4 w-full">
              ...
              <textarea id="description" v-model="activeRow.description" :disabled="modal.button === 'Read'" class="w-full border-gray-300 focus:border-blue-500 focus:ring-0 rounded-lg shadow focus:shadow-sm duration-500" resize />
            </div>
            ...
            <!-- <div class="flex flex-row mt-4">
							<Checkbox id="recurring" v-model="activeRow.recurring" class="mr-2" />
							<InputLabel for="recurring" value="Recurring" :dark="true" />
						</div> 

						<div class="flex flex-col mt-4">
							<InputLabel for="related_ids" value="Related" :dark="true" />
							<select id="related_ids" v-model="activeRow.related_ids" class="border-gray-300 focus:border-blue-500 focus:ring-0 rounded-lg shadow focus:shadow-sm duration-500">
								<option v-for="s in props.items.data" :value="s.id">{{ s.title }}</option>
							</select>
						</div> 
						
						<div class="flex flex-col mt-4">
							<InputLabel for="closure" value="Closure" :dark="true" />
							<TextInput id="closure" v-model="activeRow.closure" type="text" />
						</div> -->
          </template>
        </div>
